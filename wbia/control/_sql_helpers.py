...
def remove_old_backups(backup_dir, ext, max_keep):
    ...
    if len(path_list) > max_keep:
        ...
        for path_delete in path_delete_list:
            logger.info(
                '[ensure_daily_database_backup] Deleting old backup %r' % path_delete
            )
    ...
    if can_skip:
        ...
        if current_schema_exists:
            ...
            if current_schema_compatible:
                ...
                # this for us.
                if verbose:
                    logger.info('[_SQL] New database and a current schema found')
...
def update_schema_version(
    ...
    # Remove sqlite:/// from db.uri
    db_fpath = db.uri[len('sqlite:///') :]
    if dobackup:
    ...
    try:
        logger.info('Update path: %r ' % (valid_versions[start_index:end_index]))
        for index in range(start_index, end_index):
            next_version = valid_versions[index]
            logger.info('Updating database to version: %r' % (next_version))
